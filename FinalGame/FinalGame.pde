...
float stMillisPat = 0, stMillisLine = 0, crMillisPat, initialTimeOffset = 0;
float prevCX, prevCY, actualCX, actualCY, distMC, textTime = 0, startTime, finishTime, totalTime;
float scoreTimeEnd = 0, timeScoreEnd = 0;

ArrayList<Integer> initialPat = new ArrayList<Integer>();

int periodPat = 4000, periodLine = 800, countPat = 0, circlPos = -1, attempts_ = 0;
int posi, lvl_ = 4;
...
if(Clase.estado=="play"){
  ...
  /*
    ...
    float scoreTime = millis();
    if(scoreTime - scoreTimeEnd > 1000){
      textSize(50);
      fill (255,100,255);
      float score_ = (lvl_ + 10) - (totalTime/1000/60);
      text("Score: " + String.valueOf(score_), 500, 100);
      timeScoreEnd += 1;
      scoreTimeEnd = scoreTime;
    ...
    String s = "The only place that holds life in the Milkyway is dying.\n The quest to conquer other planets from the galaxy has intensified during the past decades.\n Sadly, humanity has not been able to find a place like home in the planetary neighborhood. \n It is your turn to find a new home for humanity.\n You need to carefully follow the path that your AI, Tonny,  gives you. \n Ready?";
    text(s,600, 350);
    fill(255);
...
void patternToDo(){
   ...
   // 5 seconds to draw the whole Pattern by computer.
  
  if(initialTimeOffset == 1){
    crMillisPat = 0;
    stMillisPat = millis();
    startTime = crMillisPat;
  }  
  
  else{crMillisPat = millis(); }
  
  //textSize(28);
  //fill(255);
  //text(crMillisPat,100,50);
...
void drawPattern(){
  //image(spaceship, handPosition.x, handPosition.y);
  /*
...
void oscEvent (OscMessage msg) {
 if(msg.checkAddrPattern("/X")==true) { 
   handPosition.x =  msg.get(0).floatValue();
   handPosition.y =  msg.get(1).floatValue();
 }
 }
